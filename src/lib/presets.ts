import {
  ...
  type FirstPersonIkOffsetName,
  type HandlingData,
  type VehiclesData,
  type CarcolsData,
  type CarvariationsData,
  type VehicleLayoutsData,
  type ModkitsData,
  type VehicleEntry,
  type MetaFileType,
...
function createDefaultIkOffsets(): Record<FirstPersonIkOffsetName, typeof ZERO_VEC3> {
  return Object.fromEntries(
    FIRST_PERSON_IK_OFFSET_NAMES.map((name) => [name, { ...ZERO_VEC3 }])
  ) as Record<FirstPersonIkOffsetName, typeof ZERO_VEC3>;
}

export const defaultVehicles: VehiclesData = {
...
function normalizeHandlingData(raw: HandlingData): HandlingData {
  ...
  return {
    ...
    fPercentSubmerged: clampNumber(raw.fPercentSubmerged, 0, 100, 85),
    ...
    fDriveInertia: clampNumber(raw.fDriveInertia, 0.01, 5, 1),
    fClutchChangeRateScaleUpShift: clampNumber(raw.fClutchChangeRateScaleUpShift, 0.01, 10, 1.5),
    fClutchChangeRateScaleDownShift: clampNumber(raw.fClutchChangeRateScaleDownShift, 0.01, 10, 1.5),
    ...
    fHandBrakeForce: clampNumber(raw.fHandBrakeForce, 0.01, 5, 0.5),
    fSteeringLock: clampNumber(raw.fSteeringLock, 10, 75, 35),
    fTractionCurveMax,
    fTractionCurveMin,
    fTractionCurveLateral: clampNumber(raw.fTractionCurveLateral, 0.1, 90, 22.5),
    fTractionSpringDeltaMax: clampNumber(raw.fTractionSpringDeltaMax, 0, 5, 0.15),
    ...
    fCamberStiffnesss: clampNumber(raw.fCamberStiffnesss, 0, 5, 0),
    fTractionBiasFront: clampNumber(raw.fTractionBiasFront, 0, 1, 0.48),
    ...
    fSuspensionUpperLimit: clampNumber(raw.fSuspensionUpperLimit, -1, 1, 0.12),
    fSuspensionLowerLimit: clampNumber(raw.fSuspensionLowerLimit, -1, 1, -0.12),
    fAntiRollBarForce: clampNumber(raw.fAntiRollBarForce, 0, 5, 0.8),
    fAntiRollBarBiasFront: clampNumber(raw.fAntiRollBarBiasFront, 0, 1, 0.5),
    fSuspensionRaise: clampNumber(raw.fSuspensionRaise, -0.5, 0.5, 0),
    fSuspensionBiasFront: clampNumber(raw.fSuspensionBiasFront, 0, 1, 0.5),
    fRollCentreHeightFront: clampNumber(raw.fRollCentreHeightFront, -2, 2, 0.3),
    fRollCentreHeightRear: clampNumber(raw.fRollCentreHeightRear, -2, 2, 0.3),
    fCollisionDamageMult: clampNumber(raw.fCollisionDamageMult, 0.01, 10, 1),
    fWeaponDamageMult: clampNumber(raw.fWeaponDamageMult, 0, 10, 1),
    fDeformationDamageMult: clampNumber(raw.fDeformationDamageMult, 0.01, 10, 0.8),
    fEngineDamageMult: clampNumber(raw.fEngineDamageMult, 0, 10, 1),
    fPetrolTankVolume: clampNumber(raw.fPetrolTankVolume, 0, 500, 65),
    fOilVolume: clampNumber(raw.fOilVolume, 0, 100, 5),
    fSeatOffsetDistX: clampNumber(raw.fSeatOffsetDistX, -2, 2, 0),
    fSeatOffsetDistY: clampNumber(raw.fSeatOffsetDistY, -2, 2, 0),
    fSeatOffsetDistZ: clampNumber(raw.fSeatOffsetDistZ, -2, 2, 0),
    nMonetaryValue: Math.round(clampNumber(raw.nMonetaryValue, 0, 1000000000, 50000)),
    strModelFlags: raw.strModelFlags?.trim() || "440010",
    strHandlingFlags: raw.strHandlingFlags?.trim() || "0",
    strDamageFlags: raw.strDamageFlags?.trim() || "0",
    aiHandling: raw.aiHandling?.trim() || "SPORTS_CAR",
    fBackEndPopUpCarImpulseMult: clampNumber(raw.fBackEndPopUpCarImpulseMult, 0, 5, 0.1),
    fBackEndPopUpBuildingImpulseMult: clampNumber(raw.fBackEndPopUpBuildingImpulseMult, 0, 5, 0.03),
    fBackEndPopUpMaxDeltaSpeed: clampNumber(raw.fBackEndPopUpMaxDeltaSpeed, 0, 10, 0.6),
  };
}
...
export const presetConfigs: Record<PresetType, PresetConfig> = {
  ...
  suv: {
    ...
    handling: {
      ...
      fAntiRollBarForce: 0.6, fSuspensionRaise: 0.0,
      fRollCentreHeightFront: 0.6, fRollCentreHeightRear: 0.6,
      fCollisionDamageMult: 0.8, fDeformationDamageMult: 0.7,
...
function cloneVec3(vec: { x: number; y: number; z: number }): { x: number; y: number; z: number } {
  return { x: vec.x, y: vec.y, z: vec.z };
}

function createVehiclesDefaultsForName(nameToken: string): VehiclesData {
  return {
    ...defaultVehicles,
    modelName: nameToken,
    txdName: nameToken,
    ...
    firstPersonIkOffsets: Object.fromEntries(
      FIRST_PERSON_IK_OFFSET_NAMES.map((key) => [
        key,
        cloneVec3(defaultVehicles.firstPersonIkOffsets[key] ?? ZERO_VEC3),
      ])
    ) as VehiclesData["firstPersonIkOffsets"],
    povCameraOffset: cloneVec3(defaultVehicles.povCameraOffset),
    povPassengerCameraOffset: cloneVec3(defaultVehicles.povPassengerCameraOffset),
    povRearPassengerCameraOffset: cloneVec3(defaultVehicles.povRearPassengerCameraOffset),
    drivers: defaultVehicles.drivers.map((driver) => ({ ...driver })),
    doorStiffnessMultipliers: defaultVehicles.doorStiffnessMultipliers.map((item) => ({ ...item })),
    txdRelationships: [{ parent: defaultVehicles.residentTxd, child: nameToken }],
    unknownNodes: [],
    unknownFileLevelNodes: [],
  };
}
...
export function createDefaultVehicle(name: string, loadedMeta?: Set<MetaFileType>): VehicleEntry {
  ...
  return {
    ...
    vehicles: createVehiclesDefaultsForName(nameToken),
