...
export function AppShell({
  ...
  // Feedback trigger — incremented each time the sidebar's early-access modal
  // wants to open the feedback dialog so the Toolbar can respond.
  ...
  const handleOpenFeedback = useCallback(() => {
    setFeedbackTrigger((prev) => ({ tab: "bug", nonce: (prev?.nonce ?? 0) + 1 }));
  }, []);
